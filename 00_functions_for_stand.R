...
#%%%%%%%%%%%%%%% Brunilda Balliu 
#%%%%%%%%%%%%%%% May 7th 2020
#%%%%%%%%%%%%%%% Functions and libraries used in the STAND project
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
create_data_dict <- function(filename){
  ...
  response_map = data_dict_raw %>%
    ...
    filter(`Field Type` %in% c("checkbox", "dropdown", "radio")) %>%   #field types with numerical coding of responses
...
abstract_feature = function(stationary_loc){
...
###### Function for CAT-MH imputation
smooth.spline2=function(daily_data,imputation_type,n_knots=NULL){
  
  obs_daily_data=daily_data %>% filter(real_cat==1)  %>% select(Date,severity)
  n_knots=min(n_knots,20)
  if(any(duplicated(obs_daily_data$severity))) CV=FALSE else CV=TRUE
  
  if(imputation_type=="fixed_knots")  
...
pred_bigstatsR=function(X, y, ind.train, ind.test, family = "normal", K = 10){
  ...
  if(family == "normal") {
    ...
    my_prediction_train <- predict(test, X, ind.row = ind.train)  
    my_prediction_test <- predict(test, X, ind.row = ind.test)  
    return(c(my_prediction_train,my_prediction_test))
  }
  
  if(family == "binomial") {
    test <- big_spLogReg(X = X, y01.train = y[ind.train], ind.train = ind.train, 
                         alphas = seq(.1,1,.1), K = K, warn = FALSE)
    pred <- predict(test, X, ind.row = ind.test)  
  }
}
...
Fb=function(actual, predicted, b=1) {
  ...
  recall=recall(actual = actual, predicted = predicted)
  
  (1+b^2) * (precision*recall)/(b^2 * precision + recall)
}


AUC_PR=function(observed, predicted){
  fg <- predicted[observed==1]
  bg <- predicted[observed==0]
  pr.curve(scores.class0 = fg, scores.class1 = bg, curve = F)$auc.integral
}
...
plot_trajectory=function(data, yaxis_lab){
  ggplot(data = data %>% mutate(real_cat = factor(x = real_cat, levels = c(0,1), labels = c("interpolated","real"))), 
    ...
    geom_line() +
    theme_bw() + 
    theme(legend.position = "top", legend.title = element_blank(), axis.title.x = element_blank(), axis.text.x = element_text(color="black")) + 
    ylab(yaxis_lab) 
}
